...
<aside class="sidebar">
  ...
  <div>
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-bold text-gray-900">Agents</h2>
      <button 
    ...
    {:else if error}
      <div class="bg-red-50 border border-red-200 rounded-lg p-3 text-red-700 text-sm">
        {error}
        <button class="block mt-2 underline" onclick={loadAgents}>Try again</button>
      </div>
    ...
    {:else}
      <div class="space-y-1">
        {#each agents as agent (agent.id)}
          <button
            class="w-full text-left px-3 py-2 rounded-md hover:bg-gray-200 transition-colors"
